...
_libgl := osx.dlopen("/System/Library/Frameworks/OpenGL.framework/OpenGL", osx.RTLD_LAZY);

get_proc_address :: proc(name: string) -> rawptr {
    res := osx.dlsym(_libgl, name);
	return rawptr(res);
}
...
// Procedures
	...
	_GetUniformLocation:      proc(program: u32, name: ^u8) -> i32 #cc_c;

// wrapper to use GetUniformLocation with an Odin string
// @NOTE: str has to be zero-terminated, so add a \x00 at the end
GetUniformLocation :: proc(program: u32, name: string) -> i32 {
    return _GetUniformLocation(program, &name[0]);;
}
...
init :: proc() {
	...
	set_proc_address(&_GetUniformLocation,     "glGetUniformLocation\x00");
